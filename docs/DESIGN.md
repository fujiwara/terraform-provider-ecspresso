...
- [x] **Phase 6**: ecspresso v2 に tfstate plugin の `optional: true` フラグ追加([kayac/ecspresso#1017](https://github.com/kayac/ecspresso/pull/1017) merge 済み)。初回 apply 時にまだ存在しない tfstate を許容するため。Provider は ecspresso の v2 branch HEAD を Go pseudo-version で参照中(細かいリリースを避けるため、ecspresso 側の正式タグ付けは provider が安定するまで待つ)
