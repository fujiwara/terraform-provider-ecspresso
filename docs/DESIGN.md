...
provider は当面これらの local fork を `go.mod replace` で参照しており、upstream マージ後に通常依存に戻す。
...
- [x] **Phase 3**: `tfstate_values` 注入機構の実装
  - tfstate-lookup に `SetOverrides` / `Empty` 追加(local fork)
  - ecspresso に `pluginInstances` / `App.PluginInstance` 追加(local fork)
...
- [ ] **Phase 4**: tfstate-lookup / ecspresso の変更を upstream PR 化、マージ後に `go.mod replace` を外す
- [ ] **Phase 5**: ecspresso.yml をパースして不足キーを警告する補助ロジック、acceptance test 整備、README 拡充

## TODO

- [ ] tfstate-lookup の override API の upstream PR
- [ ] ecspresso の plugin instance registry の upstream PR
